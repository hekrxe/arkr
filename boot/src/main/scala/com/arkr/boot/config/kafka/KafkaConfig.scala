...
class KafkaConfig extends InitializingBean {
  ...
  @Value("kafka.consumer.listener.concurrency:10")
  private var CONCURRENCY: Int = _
  ...
  def producerConfig: util.Map[String, Object] = {
    ...
    map.put(ProducerConfig.BATCH_SIZE_CONFIG, 4096)
    map.put(ProducerConfig.LINGER_MS_CONFIG, 1)
    map.put(ProducerConfig.BUFFER_MEMORY_CONFIG, 40960)
  ...
  def consumerConfig: util.Map[String, Object] = {
    ...
    map.put(ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG, classOf[StringSerializer])
    map.put(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG, classOf[StringSerializer])
